...
A Kubernetes operator that sleeps `Deployments`/`StatefulSets` on a schedule and
wakes them on demand, restoring their exact prior replica count. It mutates only
`/spec/replicas`, so it is safe alongside GitOps tooling. See the
[project README](../README.md) for the elevator pitch.
...
| **Checkpoint** | The out-of-band Secret storing each workload's pre-sleep replica count, for exact restore. |
