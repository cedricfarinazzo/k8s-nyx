...
- `charts/k8s-nyx-chart/` — the Helm chart (CRD, RBAC, operator Deployment,
  optional webhook): the single install path, published to GHCR as an OCI artifact.
- `.github/workflows/` — `ci.yml` (build/test/lint/docker + helm lint/template +
  kind install & `helm test`) and `release.yml` (tag-triggered image + chart publish).
